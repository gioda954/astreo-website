...
export default async function HistoryPage({ searchParams }: { searchParams: Promise<{ campagna?: string }> }) {
  ...
  return <>
    ...
    {selected ? <>
      ...
      <div className="table-wrap"><table><thead><tr><th>Data</th><th>Cliente</th><th>Azione</th><th>Casse</th><th>Totale</th><th>Contatto</th><th>Pagamento</th><th>Consegna</th></tr></thead><tbody>{orders.map((order) => <tr key={order.id}>
        <td>{new Date(order.created_at).toLocaleDateString("it-IT")}</td><td>{order.customer_name}</td><td><Link className="order-action" href={`/crm/ordini/${order.id}`}>Vedi dettagli / contatta</Link></td><td>{order.cases_quantity}</td><td>{formatEuro(order.expected_total_cents)}</td>
