...
export default async function ThankYouPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="app-shell"><header className="topbar"><Link className="brand" href="/"><img src="/images/logo.jpg" alt="Logo Astreo"/><span>Astreo</span></Link></header><div className="page"><section className="card" style={{maxWidth:760,margin:"0 auto"}}>
    <p className="eyebrow">Ordine ricevuto</p><h1>Grazie, {order.customer_name.split(" ")[0]}!</h1><p className="lead">Grazie per aver partecipato all’edizione {order.vintage}. Ti contatteremo presto per concordare pagamento e {order.fulfillment_type === "spedizione" ? "spedizione" : "ritiro"}.</p>{order.fulfillment_type === "ritiro" && <div className="pickup-notice"><strong>Ritiro a Brogliano</strong><p>Via Spesse 5, 36070 Brogliano (VI), previo accordo con Giovanni Dal Lago.</p></div>}
